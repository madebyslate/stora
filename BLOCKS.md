...
| `LogoWall` | 🟢 done | `home`, `about-us` | 352 B inline (wspólny obserwator) | Sekcja pod hero na Home i po zespole na About Us, z jednego współdzielonego wpisu treści. Trzy odstępstwa od Figmy, każde z pomiarem — `LogoWall.spec.md`. Green wchodzi do palety z limitem „tylko ≥ 24 px". |
| `ServiceCards` | 🟡 in progress | `home` | **0 B** | Trzy obszary jako pełnoekranowe kafle, rozwijane bez ani jednej linii JS. Geometria z ramki, kontrast zmierzony pod glifami (najgorszy 5,01 przy progu 4,5) — kosztem dwóch wartości z Figmy. Zostały snapshoty i Lighthouse. `ServiceCards.spec.md`. |
| `OurProcess` | 🟡 in progress | `home` | **0 B** | Cztery kroki w sekcji przypiętej na scroll, sterowane jedną `view-timeline` — bez linii JS. Zdjęcia wymieniają się **taśmą**, nie przenikaniem: panel wychodzący i wchodzący liczą się z tego samego postępu, więc stykają się co do piksela pod dowolną krzywą. Tekst wychodzi, zanim zdjęcia ruszą, i wchodzi, gdy skończą. Trzy zasady, na których to stoi, i przemiatanie 101 pozycji scrolla, które ich pilnuje — `OurProcess.spec.md`. |
| `TeamGrid` | 🟡 in progress | `home`, `about-us` | **0 B** | Cztery portrety ze współdzielonej listy; About Us nadpisuje tylko nagłówek. Jeden kafel zawsze wyróżniony — najechanie przekazuje wyróżnienie dalej, bez linii JS. Stan spoczynku i wyróżniony to **jedno jednolite `scale()`** na jednym elemencie, bo kafel niesie proporcję portretu (36/43). Zmierzone: portret 180,02 × 215,01 przy 1440. Kosztuje 7 px wysokości kafla — powód w `TeamGrid.spec.md`. Zostały snapshoty, Lighthouse i kontrast (odłożone na prośbę klienta). |
| `WiderTeam` | 🟡 in progress | `about-us` | **0 B** | Dziewięć portretów w pięciu kolumnach, bez LinkedIna i bez domyślnie aktywnego kafla. Zostało porównanie wizualne i pełny pakiet testów, odłożone na prośbę klienta. |
| `MarketSlider` | 🟡 in progress | `home` | 470 B gz inline | Slider bez JS: grupa radio → `--active` → dwie taśmy (zdjęcia i pary strzałek). Geometria odtworzona z kadru rysowanego w skali 0,6 — skala potwierdzona trzema niezależnymi drogami. Zmierzone na zbudowanej stronie przy 1440: skrzynka 1360, pasmo nagłówka 197, zdjęcie 640 × 640 na x = 400, przyciski 40 × 40 z 1 px przerwy, dosunięte do prawej krawędzi zdjęcia i do linii pod tytułem, opis 22/28/500 w mierze 296, opis → przycisk 40. Nowy stopień skali `--text-claim`. Gest przeciągania (palec i mysz) + trzy zamknięte drogi na wyciągnięcie zdjęcia poza stronę. Opis wjeżdża **słowo po słowie** przy każdej zmianie slajdu, zdjęcie odmaskowuje się z lewej. Zostały snapshoty, Lighthouse i treść slajdów 2–3. `MarketSlider.spec.md`. |
| `AudienceTabs` | 🟡 in progress | `home`, `about-us` | **0 B** | Współdzielona 1:1 sekcja „Store energy at large scale”. Trzy pytania, trzy zdjęcia, trzy calle — przełącznik bez JS: radio w środku labela, więc stan aktywny i fokus to po jednej regule, a nie po jednej na pozycję. Całe zdjęcie jest linkiem, przycisk w rogu to tylko afordancja (`Button as="span"`). Nieaktywne panele są `visibility: hidden`, więc w kolejności tabulacji jest dokładnie jeden link — zmierzone. Zmierzone przy 1440: pasmo 1360 × 500, połowy 675 z 10 px przerwy, tło `rgb(241,242,235)`, pytanie 40/500/-0,8 px w rytmie 80,24, przycisk 40 wysokości w wcięciu 39,7 / 39,8, nagłówek 56/64 z 48 do pasma i 120 nad nim, pod nim zero. Zostały snapshoty i Lighthouse (odłożone na prośbę klienta). `AudienceTabs.spec.md`. |
| `PageHero` | 🟢 done | `about-us`, `brokerage`, `develop-to-sell`, `develop-to-hold` | **0 B** | Hero podstrony: zdjęcie, `<h1>` 72/500 i linia 28/36 wyśrodkowane 72 px nad stopą bloku, wysokość `min(900px, 100svh)`. Efekt „następna sekcja najeżdża na hero" to `position: sticky` + `z-index: -1` — bez linii JS, i wyłączony przy `prefers-reduced-motion`. Zmierzone przy 1440 na zbudowanej stronie: blok 900, nagłówek 72/72 na 700, linia na 792 (odstęp 20), stopa 828 → 900 (72). Własne scrimy, głębsze niż na home — najgorszy zmierzony kontrast pod glifami: nagłówek 3,39, linia 5,08, nawigacja 5,08 (4 strony × 5 szerokości). Snapshoty desktop + mobile, Lighthouse 100/100/100/100 na wszystkich czterech adresach. `PageHero.spec.md`. |
| `AboutStory` | 🟡 in progress | `about-us` | **717 B gz inline** | Pierwsza sekcja pod hero: sześć dostarczonych zdjęć wokół zdania 32/40/500. Podczas scrolla istniejący `PageHero` schodzi do kwadratu 156 × 156 i przekazuje kadr statycznemu zdjęciu w kompozycji; kontrskala osi zachowuje proporcje fotografii także na pionowym ekranie. Po morfowaniu kompozycja jest przypięta przez `100svh`: każde zdjęcie ma osobny próg wejścia, a słowa przechodzą kolejno z 0,5 do pełnego krycia przed zwolnieniem sekcji. Scena jest wyśrodkowana pionowo w wysokim viewporcie, a na niskim ogranicza wysokość do dostępnego miejsca. Bez JS i przy ograniczeniu ruchu blok nie tworzy dodatkowej wysokości i pokazuje stan końcowy. Geometria desktopowa przeliczona ze zrzutu 1016 × 682 do ramki 1440; przebieg sprawdzony ręcznie przy 2294 × 1414, 390 × 1200 i 390 × 700, bez poziomego przelewu. `pnpm verify`: 0 błędów / warningów; snapshoty i Lighthouse odłożone na prośbę klienta. `AboutStory.spec.md`. |
...
| `Cta` | 🟡 in progress | wszystkie | **0 B** | Domknięcie strony, **globalne** (`site.cta`), nie blok — renderuje je `BaseLayout` wewnątrz `<main>`. Zmierzone przy 1440: padding 120/120, znak 116, znak → nagłówek 48, nagłówek 72/72, nagłówek → linia 28, linia 20/28/500, linia → przycisk 48. Przycisk odwrócony (ciemne tło, biały tekst) przez nadpisanie jego własnych tokenów na wrapperze; na hoverze dzieli wspólne odmaskowanie Green. Dwa nowe stopnie skali: `--text-standfirst` i `--text-detail`. Zostały snapshoty, Lighthouse i kontrast (odłożone na prośbę klienta). `Cta.spec.md`. |
| `Footer` | 🟡 in progress | wszystkie | **0 B** | Trzy kolumny rozdzielone kreskami na pełną wysokość bloku — dlatego padding siedzi na kolumnach, a nie na kontenerze: kreska jest krawędzią kolumny, więc sięga bordera i stopy z konstrukcji, a nie z dobranej pary liczb. Nawigacja to `site.navigation`, ta sama tablica co w headerze. Zmierzone przy 1440: kreski 463,2 / 759,9 (projekt 463 / 759), padding 64/64, rytm nawigacji 40, grupa → grupa 72, podział kolumn kontaktowych 256. Etykiety grup mają 3,04:1 — wartość z projektu, pytanie w spec-u. `Footer.spec.md`. |
...
>
> Sekcja ma 560 vh, nie 500: 1,6 ekranu na krok zamiast jednego. Przy jednym
> zamiana zdjęć miała 38 vh, a połowa nazwy 11 — za mało, żeby czytać się jako ruch,
> a nie jako cięcie. Krzywa zeszła z `--ease-out-expo` na `--ease-in-out`, bo na osi
> sterowanej scrollem expo wrzuca dziewięć dziesiątych drogi w pierwszą trzecią
> odcinka: to jest właśnie to „przeskakuje".
>
